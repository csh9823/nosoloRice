	...
	<div class="container">
		<div class="row">
			<div class="row">
				...
				<div class="col mt-5">
					<div class="mt-3 fs-2 fw-bold" style="color: #C93C3C">사업 문의 답변</div>
						<br>
					    ...
					    <c:if test="${empty adminBusinessInquiryList}">
					...
					<div class="row">
					    <div class="col" id="inquiryCol">
					        <c:if test="${not empty adminBusinessInquiryList}">
					            <c:forEach var="b" items="${adminBusinessInquiryList}">
					                <div class="row text-center inquiryRow" onclick="location.href='businessInquiryDetail?businessInquiryNo=${b.businessInquiryNo}'"
					                style="text-decoration:none; color:#333; cursor:pointer;">
					                    <div class="col-2 my-3">${b.businessInquiryNo}</div>
					                    <div class="col-2 my-3">${b.businessDivision}</div>
					                    <div class="col-3 my-3">${b.businessTitle}</div>
					                    <div class="col-2 my-3">${b.businessId}</div>
					                    <div class="col-3 my-3"><fmt:formatDate value="${b.businessInquiryRegDate}" pattern="yyyy-MM-dd" /></div> 
					                </div>
					            </c:forEach>
					        </c:if>    
					    </div>
									...
									<ul class="pagination justify-content-center">
										...
										<c:if test="${ startPage > PG }">
											<li class="page-item"><a class="page-link"
												href="adminBusinessInquiryList?pageNum=${ startPage - PG }">Pre</a></li>
										</c:if>
										...
										<c:forEach var="i" begin="${startPage}" end="${endPage}">
											...
											<c:if test="${i != currentPage }">
												<li class="page-item"><a class="page-link"
													href="adminBusinessInquiryList?pageNum=${i}">${i}</a></li>
											</c:if>
										</c:forEach>

										<c:if test="${ endPage < pageCount }">
											<li class="page-item"><a class="page-link"
												href="adminBusinessInquiryList?pageNum=${ startPage + PG }">Next</a>
											</li>
										</c:if>
									</ul>
								</nav>
